...
class ParkManageController extends Controller
{
    ...
    private function createRecurringBlock($zone, $validated)
    {
        ...
        $recurringBlock = ParkManage::create([
            ...
            'weekly_days' => json_encode($validated['weekly_days']),
    ...
    public function getFutureBlocks(Request $request)
    {
        ...
        return response()->json($blocks->map(function($block) {
            return [
                'id' => $block->id,
                'zone_name' => $block->zone->zone_name ?? "Zone P{$block->zone->zone_number}",
                'zone_number' => $block->zone->zone_number,
                'zone_type' => $block->zone->zone_type,
                'reason' => $block->reason,
                'date' => $block->date,
                ...
                'weekly_days' => $block->is_recurring ? json_decode($block->weekly_days) : null,
            ...
            $blocks = $query->get()->map(function($block) {
                try {
                    ...
                    return [
                        ...
                        'end_time' => $block->end_time,
                        'schedule_type' => $block->schedule_type,
                        'weekly_days' => json_decode($block->weekly_days ?? '[]', true),
                        'status' => $status,
