...
class ParkingZoneController extends Controller
{
    ...
    public function getAllZoneStatuses()
    {
        ...
        app(\App\Http\Controllers\ParkManageController::class)->processScheduledBlocks();
        app(\App\Http\Controllers\ParkManageController::class)->processRecurringBlocks();
            ...
            if($zoneCoords){
                ...
                if ($futureBlock) {
                    $zoneArr['future_block'] = [
                        ...
                        'weekly_days' =>  $futureBlock->is_recurring
                            ? (is_array($futureBlock->weekly_days)
                                ? $futureBlock->weekly_days
                                : (is_string($futureBlock->weekly_days) && strlen($futureBlock->weekly_days) > 0
                                    ? (json_decode($futureBlock->weekly_days, true) ?: [])
                                    : []))
                            : null,
